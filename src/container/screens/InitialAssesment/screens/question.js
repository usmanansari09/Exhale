...
class InitialScreen extends Component {
  ...
  submitSitToStand = async (e) => {
    ...
    if (!_isEmpty(this.state.option)) {
      this.props.setOptionValues(`questionId${this.props.item?.id}`, this.state.option);
    ...
    return (
      <>
        <Wrapper>
          ...
          <form className="my-3" onSubmit={this.submitSitToStand} style={{ width: '440px' }}>
            {item?.allow_many
              ...
              : item?.choices?.map((element) => {
                  return (
                    <Row>
                      <input
                        type="radio"
                        onChange={() => this.handleRadioButtonChange(element.id)}
                        checked={this.state.option[`id`] === element.id}
                      />
