...
class Login extends Component {
  ...
  submitNewPassword = async (e) => {
    ...
    const { resetConfirmPassword, otpResponse } = this.state;

    const resetPsd = this.state.resetPassword;

    console.log(otpResponse);
    const resetPasswordError = passwordValidator(resetPsd);
    if (resetPasswordError) this.setState({ resetPasswordError });

    let resetPasswordError1 = "";
    if (resetPsd !== resetConfirmPassword)
      resetPasswordError1 = "Password do not match";
    if (resetPasswordError1) this.setState({ resetPasswordError1 });

    if (!resetPasswordError1 && !resetPasswordError) {
      this.props.loading(true);
      let response = await resetPassword(
        {
          new_password: resetPsd,
